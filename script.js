...
window.BrstWorks = {
    ...
    prevTrack,
    scrollToSection
};// Small utilities: mobile menu toggle + logo fallback
(function () {
  // Mobile menu toggle
  document.addEventListener('DOMContentLoaded', function () {
    ...
    document.addEventListener('click', function (evt) {
      if (!navMenu.contains(evt.target) && !toggle.contains(evt.target) && navMenu.classList.contains('active')) {
  ...
  // Logo fallback (uses #siteLogo)
  (function () {
    const img = document.getElementById('siteLogo');
    if (!img) return;
    const logoPath = img.getAttribute('src') || 'assets/images/logo.png';

    function setSvgFallback() {
      const svg = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64" width="64" height="64" preserveAspectRatio="xMidYMid meet" role="img" aria-label="logo BrstWorks">
        <defs><linearGradient id="bwGrad" x1="0" x2="1" y1="0" y2="1"><stop offset="0" stop-color="#4ecdc4"/><stop offset="1" stop-color="#ff6b6b"/></linearGradient></defs>
        <rect width="64" height="64" rx="12" fill="url(#bwGrad)"/>
        <path d="M16 44 L28 20 L36 40 L48 18" stroke="#fff" stroke-width="4" fill="none" stroke-linecap="round" stroke-linejoin="round"/>
    ...
    if (location.protocol === 'file:') {
      img.addEventListener('error', setSvgFallback);
      setTimeout(() => { if (!img.naturalWidth) setSvgFallback(); }, 250);
      return;
    }

    fetch(logoPath, { method: 'HEAD' }).then(res => {
      if (!res.ok) setSvgFallback();
    }).catch(() => {
      fetch(logoPath, { method: 'GET' }).then(res => {
        if (!res.ok) setSvgFallback();
      }).catch(() => setSvgFallback());
    });
  })();
})();
